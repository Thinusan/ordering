...
//    }else{
//       $add_food_query = mysqli_query($conn, "INSERT INTO `food_tbl`(food_name, price, image, special_day, availability) VALUES('$name', '$price', '$image', '$special_day', '$availability')") or die('query failed');
...
if(isset($_GET['delete'])){
   ...
   $delete_image_query = mysqli_query($conn, "SELECT image FROM `food_tbl` WHERE food_id = '$delete_id'") or die('query failed');
   $fetch_delete_image = mysqli_fetch_assoc($delete_image_query);
   unlink('../images/'.$fetch_delete_image['image']);
   mysqli_query($conn, "DELETE FROM `food_tbl` WHERE food_id = '$delete_id'") or die('query failed');
...
if(isset($_POST['update_food'])){
   ...
   mysqli_query($conn, "UPDATE `food_tbl` SET food_name = '$update_name', category = '$update_category', price = '$update_price', special_day= '$update_special_day', availability= '$update_availability' WHERE food_id = '$update_f_id'") or die('query failed');
   ...
   if(!empty($update_image)){
      if($update_image_size > 2000000){
         ...
         mysqli_query($conn, "UPDATE `food_tbl` SET image = '$update_image' WHERE food_id = '$update_f_id'") or die('query failed');
   ...
   ?>
                     ...
                     <div class="box-container">
                        ...
                        <div class="box">
                           ...
                           <a href="admin_foods.php?update=<?php echo $fetch_foods['food_id']; ?>" class="option-btn">update</a>
                           <a href="admin_foods.php?delete=<?php echo $fetch_foods['food_id']; ?>" class="delete-btn" onclick="return confirm('Confirm delete?');">delete</a>
                        </div>
                        <?php
                           }
                        }
...
<section class="edit-product-form">
   <?php
      if(isset($_GET['update'])){
         ...
         $update_query = mysqli_query($conn, "SELECT * FROM `food_tbl` WHERE food_id = '$update_id'") or die('query failed');
         if(mysqli_num_rows($update_query) > 0){
            while($fetch_update = mysqli_fetch_assoc($update_query)){
   ?>
   <form action="" method="post" enctype="multipart/form-data">
      <input type="hidden" name="update_f_id" value="<?php echo $fetch_update['food_id']; ?>">
